...
import EmptyModelPage from './EmptyModelpage';
...
const MyModel = () => {
    ...
    return (
       ...
       myModels.length === 0 ? <EmptyModelPage></EmptyModelPage> :
    (
        <div className="w-full bg-[#0b0e14] min-h-screen pt-24 pb-10 px-6">
            ...
            {
                loading ?
                    <div className='inset-0 z-[100] flex justify-center mt-40'><Loader></Loader></div> :
                        ...
                        <AnimatePresence>
                            {myModels.map((model, index) => (
                                <motion.div
                                    ...
                                    className="bg-[#0d1117] border border-white/10 rounded-2xl overflow-hidden hover:border-blue-500/50 transition-all group shadow-xl"
                                >
                                    {/* Model Image Section */}
                                    <div className="relative h-48 w-full overflow-hidden bg-white/5">
                                        <img 
                                            src={model.image || 'https://via.placeholder.com/400x200?text=AI+Model'} 
                                            alt={model.name}
                                            className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                                        />
                                        <div className="absolute top-3 left-3">
                                            <span className="text-[10px] font-mono text-white/50 bg-black/60 backdrop-blur-md px-2 py-1 rounded">
